...
export const FormalLetterCard: React.FC<FormalLetterCardProps> = ({
  ...
  return (
    <div
      ...
      className={`print-container relative w-full max-w-[820px] mx-auto min-h-[1050px] ${themeStyles.bgBase} text-stone-800 rounded-xl shadow-2xl p-3 sm:p-5 md:p-6 border-3 sm:border-4 ${themeStyles.outerBorder} overflow-hidden transition-all duration-300 flex flex-col justify-between`}
    >
      {/* Traditional Double Inner Border with Alpona Corners */}
      <div
        className={`relative flex-1 border sm:border-2 ${themeStyles.borderPrimary} border-opacity-70 p-3 sm:p-5 md:p-6 rounded-lg flex flex-col justify-between`}
      >
        ...
        <header className="relative text-center z-10 pt-1 pb-1 px-1">
          ...
          <div className="flex flex-row items-center justify-center gap-3 sm:gap-4 my-1.5">
            <AssociationCrest color={themeStyles.alponaColor} size={54} className="shrink-0" />
            <div className="text-left">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-semibold tracking-wider uppercase bg-amber-100 text-amber-900 border border-amber-300/80">
                <GraduationCap className="w-3.5 h-3.5 text-amber-800 shrink-0" />
                অফিসিয়াল আমন্ত্রণ পত্র • Formal Invitation
              </span>
              <h1
                className={`text-xl sm:text-2xl md:text-3xl font-extrabold font-serif-bn tracking-tight ${themeStyles.primaryText} leading-tight mt-0.5`}
              >
                বিরল উপজেলা ছাত্রকল্যাণ সমিতি
              </h1>
              <p className="text-xs sm:text-sm font-semibold text-stone-700 font-serif-bn tracking-wide leading-tight">
          ...
          {/* Ornamental Divider */}
          <BengaliDivider color={themeStyles.alponaColor} className="my-1.5 sm:my-2" />

          {/* Event Grand Title Banner */}
          <div className="mt-1 mb-2 px-1">
            <div
              className={`inline-block w-full sm:w-auto px-4 sm:px-8 py-2 rounded-lg bg-gradient-to-r ${themeStyles.headerGradient} text-white shadow-xs border-b-2 border-amber-400`}
            >
              <div className="flex items-center justify-center gap-1.5 text-amber-200 text-xs font-medium tracking-wide">
                <Sparkles className="w-3.5 h-3.5 animate-pulse text-amber-300 shrink-0" />
                <span>বার্ষিক মহাসম্মেলন ও প্রীতি সম্মিলন</span>
                <Sparkles className="w-3.5 h-3.5 animate-pulse text-amber-300 shrink-0" />
              </div>
              <h2 className="text-base sm:text-xl md:text-2xl font-bold font-serif-bn mt-0.5 tracking-wide leading-tight">
        ...
        {/* ================= FORMAL LETTER BODY ================= */}
        <main className="relative z-10 flex-1 flex flex-col justify-between text-stone-800 font-serif-bn px-1 sm:px-3 text-left sm:text-justify leading-relaxed py-1">
          {/* Formal Salutation */}
          <div className="mb-2 flex items-baseline gap-1.5 flex-wrap border-l-3 border-amber-600 pl-2.5 py-0.5">
          ...
          {/* Opening Paragraph */}
          <p className="text-xs sm:text-[13.5px] text-stone-800 indent-5 leading-relaxed mb-2">
          ...
          <div
            className={`my-1.5 p-2.5 sm:p-3 rounded-md bg-stone-50/90 border-l-4 ${themeStyles.borderPrimary} border shadow-2xs relative`}
          >
          ...
          <section className="my-2" aria-label="অনুষ্ঠানের সময়সূচি">
            <div className="text-center mb-1.5">
              <span className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-amber-900 bg-amber-100/90 px-3 py-0.5 rounded-full border border-amber-300 shadow-2xs">
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-3 mt-1.5">
              {/* Date Card */}
              <div className="flex flex-col items-center justify-center p-2 sm:p-2.5 rounded-lg bg-white border border-stone-200 shadow-2xs text-center">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-1 ${themeStyles.badgeBg}`}>
                  <Calendar className="w-4 h-4" />
                </div>
                <span className="text-[10px] font-medium text-stone-500 font-sans-bn">
                  তারিখ ও বার
                </span>
                <strong className="text-xs sm:text-sm font-bold text-stone-900 font-serif-bn leading-tight">
                  ২০ সেপ্টেম্বর ২০২৬
                </strong>
                <span className="text-[10.5px] font-semibold text-amber-700 font-sans-bn">
                  রবিবার
                </span>
              </div>
              ...
              <div className="flex flex-col items-center justify-center p-2 sm:p-2.5 rounded-lg bg-white border border-stone-200 shadow-2xs text-center">
                ...
                <span className="text-[9.5px] font-medium text-stone-600 font-serif-bn">
          ...
          <section
            className={`my-2 p-2.5 sm:p-3 rounded-lg ${themeStyles.boxBg} shadow-2xs`}
            aria-label="আয়োজনের মূল আকর্ষণ ও কর্মসূচি"
          >
            <div className="flex items-center justify-center gap-1.5 mb-1.5">
              <span className="text-xs sm:text-sm font-bold text-stone-900 font-serif-bn flex items-center gap-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
              {eventHighlights.map((item, idx) => (
                <div
                  key={idx}
                  className="flex items-start gap-2 bg-white/95 p-1.5 sm:p-2 rounded-md border border-amber-200/80 shadow-2xs"
                >
                  <div className="mt-0.5 shrink-0 p-1 rounded bg-amber-50 border border-amber-200">
                    {item.icon}
                  </div>
                  <div>
                    <h4 className="text-[11.5px] sm:text-xs font-bold text-stone-900 font-serif-bn leading-tight">
                      {item.title}
                    </h4>
                    <p className="text-[10px] sm:text-[10.5px] text-stone-600 font-sans-bn leading-tight mt-0.5">
                      {item.desc}
                    </p>
                  </div>
                </div>
          ...
          <div className="my-1.5">
            <p className="text-xs sm:text-[13px] text-stone-800 indent-5 leading-relaxed mb-1.5">
            ...
            <div className="text-center my-1 py-1.5 px-3 rounded-md bg-stone-100/80 border border-stone-200/90">
              <p className="text-[11px] sm:text-xs font-semibold text-amber-950 font-serif-bn italic">
            ...
            <div className="text-center mt-1.5">
              <span
                className={`inline-flex items-center gap-1.5 text-sm sm:text-base font-bold ${themeStyles.primaryText} font-serif-bn`}
              >
        ...
        <footer className="relative z-10 mt-2 pt-2 border-t border-stone-200/90 font-serif-bn">
          <div className="flex flex-row items-center justify-between gap-2 sm:gap-4">
            ...
            <div className="w-1/3 text-left text-[10px] sm:text-[11px] text-stone-500 space-y-0.5">
              ...
              <p className="text-[9.5px] sm:text-[10.5px] leading-tight text-stone-500">
                হাজী মোহাম্মদ দানেশ বিজ্ঞান ও প্রযুক্তি বিশ্ববিদ্যালয় (হাবিপ্রবি), দিনাজপুর।
              </p>
              <div className="inline-flex items-center gap-1 text-[9px] sm:text-[9.5px] text-amber-800 bg-amber-50 px-2 py-0.5 rounded border border-amber-200/80">
            ...
            <div className="w-1/3 flex items-center justify-center">
              <OfficialSealStamp theme={theme} size="md" />
            </div>

            {/* Right: Formal Signatory Block */}
            <div className="w-1/3 text-right flex flex-col items-end">
              <p className="text-[10.5px] sm:text-xs text-stone-600 italic">
              ...
              <p className="text-[9.5px] sm:text-[10px] text-stone-600 font-medium leading-tight">
                হাবিপ্রবি, দিনাজপুর
              </p>
            </div>
          </div>
